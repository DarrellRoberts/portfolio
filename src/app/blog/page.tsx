    ...
    <section style={{backgroundColor: themeStyles.bg}} className={styles.homepageCon}>
        <div className={styles.titleCon}>
          <div className={styles.darrellCon}>
            <h1 style={{color: themeStyles.secondaryText}} className="heroTitle">
                Blog
            </h1>
            <div className={styles.postsCon}>
                {EntriesArray.map((item, i) => (
                <Link key={i} href={`/blog/${item.id}`}>
                    <div style={{borderColor: themeStyles.secondaryText}} className={styles.rotateBorder}></div>
                        <div className={styles.posts}>
                            <h2 style={{color: themeStyles.mainText}}>{item.title}</h2>
                            <h2 style={{color: themeStyles.mainText}}>Posted: {item.date}</h2>
                        </div>
                </Link>
                ))}
            </div>
          </div>
          <Image className={styles.mail} src={isLightTheme ? "/assets/blog.svg" : "/assets/blogDM.svg"} alt="cog" width="250" height="250" />
        </div>
      </section>
  </div>
  )
